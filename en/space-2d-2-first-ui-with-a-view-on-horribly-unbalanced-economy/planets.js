function Planet(x, y) {
	...
	this.step = function () {
		...
		to_build.forEach((count, kind) => {
			...
			this.owner.bids.add(this, kind, count, this.incomes[building_kind]*count, this.prices[2]);
		});
	};
	this.attacked = function (player, attacking_planets, attacking_robots, attacking_robots_sum) {
		...
		this.updateWarDist();
		if (colonising) {
			this.colonise(player, true);
		}
		// else console.log("planet ",this.name, "attacked");
	};
	this.colonise = function (player, attacked) {
		if (!attacked) {
			this.owner = player;
			this.store[2] = 0;
			this.updateWarDist();
			// console.log("c1", this.war_dist);
		}
		planets.ensureSquareSurrounded(Math.floor(this.x / 10), Math.floor(this.y / 10));
		// add name
		if (!player.planets) {
			this.name = player.name + randomFrom([' Home', ' Prime', ' First'])
			player.planets++;
		} else {
			player.planets++;
			this.name = player.name + ' ' + player.planets;
		}
		// add 3 buildings
		for (var i = 0; i < 3; i++) {
		...
		this.updatePrices();
		// console.log("p", this.prices, this.incomes);
		...
		// console.log("planet ",this.name, "colonised");
		timeline.add(this);
	};
	this.getText = function (i) {
		if (this.owner) {
			return [ this.name,
			`${i} (${this.war_dist}) - ${this.value}`,
			'Q: ' + this.quality,
			'P: '+this.prices,
			'I: ' + this.incomes,
			'S: ' + this.store,
...
planets = {
	...
	ensureSquareSurrounded: function (x0, y0, delta=1) {
