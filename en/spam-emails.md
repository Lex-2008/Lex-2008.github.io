...
<div id="auto"><table> <!-- ABOVE -->
in generated content, there will be </table><div class="old">
</div></div><!-- BELOW -->
...
<a name="script"></a>
	...
	grep --no-filename -A1 'listed at rbl.rbldns.ru' /var/log/mail.log$1 | sed -rn '/from=/{s/^(...) (..) .* from=<([^>]*)>, .* relay=/\1@\2@\3<\/td><td>from /;p}' | LC_ALL=C sort -u -t '@' -k 1,1Mr -k 2,2r -k 4 -k 3 | sed -n 's/@/ /; s/@/%/; H; g;/^\(... ..\)\n\1/!{s/.*\n\(.*\)%.*/<tr><td>\1<\/td><\/tr>/;p}; g;s/.*%\(.*\)/<tr><td>\1<\/td><\/tr>/;p; g;s/.*\n\(.*\)%.*/\1/;h'
<script src="/microlight.js"></script>
<script>
document.body.onload=function(){
	t=window.performance.timing;
	document.getElementById('stat').innerHTML='Total <b>'+
		(document.getElementById('auto').innerHTML.split('@').length-1)+
